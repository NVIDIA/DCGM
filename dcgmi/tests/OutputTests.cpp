...
TEST_CASE("DCGMI: CommandOutputController")
{
    ...
    SECTION("HelperDisplayValue")
    {
        CommandOutputController coc;

        SECTION("DCGM_STR_BLANK conversion")
        {
            std::string result = coc.HelperDisplayValue(DCGM_STR_BLANK);
            REQUIRE(result == "Not Specified");
        }

        SECTION("DCGM_STR_NOT_FOUND conversion")
        {
            std::string result = coc.HelperDisplayValue(DCGM_STR_NOT_FOUND);
            REQUIRE(result == "Not Found");
        }

        SECTION("DCGM_STR_NOT_SUPPORTED conversion")
        {
            std::string result = coc.HelperDisplayValue(DCGM_STR_NOT_SUPPORTED);
            REQUIRE(result == "Not Supported");
        }

        SECTION("DCGM_STR_NOT_PERMISSIONED conversion")
        {
            std::string result = coc.HelperDisplayValue(DCGM_STR_NOT_PERMISSIONED);
            REQUIRE(result == "Insf. Permission");
        }

        SECTION("Tab character replacement")
        {
            std::string result = coc.HelperDisplayValue("Brando\tSando");
            REQUIRE(result == "Brando Sando");
        }

        SECTION("Newline and tab character replacement")
        {

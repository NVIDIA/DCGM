...
# Unacceptable rate of volatile single bit errors
DCGM_FR_VOLATILE_SBE_DETECTED = 5
...
# Unacceptable total page retirements due to uncorrectable errors
DCGM_FR_RETIRED_PAGES_DBE_LIMIT = 8
...
# Disagreement in GPU count between /dev and NVML
DCGM_FR_DEVICE_COUNT_MISMATCH = 20
...
# Unstable connection to nv-hostengine (daemonized DCGM)
DCGM_FR_HOSTENGINE_CONN = 26
...
# The value for the specified field is above the threshold
DCGM_FR_FIELD_THRESHOLD = 34
# The value for the specified error field is above 0
DCGM_FR_FIELD_VIOLATION_DBL = 35
# The value for the specified field is above the threshold
DCGM_FR_FIELD_THRESHOLD_DBL = 36
DCGM_FR_UNSUPPORTED_FIELD_TYPE = 37  # Field type cannot be supported
# The value for the specified field is above the threshold
DCGM_FR_FIELD_THRESHOLD_TS = 38
# The value for the specified field is above the threshold
DCGM_FR_FIELD_THRESHOLD_TS_DBL = 39
...
# More than 100 CRC errors are happening per second
DCGM_FR_NVLINK_CRC_ERROR_THRESHOLD = 70
# NVLink error for a field that should always be 0
DCGM_FR_NVLINK_ERROR_CRITICAL = 71
# The enforced power limit is too low to hit the target
DCGM_FR_ENFORCED_POWER_LIMIT = 72
...
# Pending page retirements due to a DBE
DCGM_FR_DBE_PENDING_PAGE_RETIREMENTS = 83
DCGM_FR_UNCORRECTABLE_ROW_REMAP = 84  # Uncorrectable row remapping
DCGM_FR_PENDING_ROW_REMAP = 85  # Row remapping is pending
# P2P copy test detected an error writing to this GPU
DCGM_FR_BROKEN_P2P_MEMORY_DEVICE = 86
# P2P copy test detected an error writing from this GPU
DCGM_FR_BROKEN_P2P_WRITER_DEVICE = 87
...
# EUD process remains running after the plugin considers it finished
DCGM_FR_EUD_ZOMBIE = 93
# EUD process exited with a non-zero exit code
DCGM_FR_EUD_NON_ZERO_EXIT_CODE = 94
DCGM_FR_EUD_TEST_FAILED = 95  # EUD test failed
# We cannot write a file in this directory.
DCGM_FR_FILE_CREATE_PERMISSIONS = 96
...
# Expected nvlinks up per nvswitch */
DCGM_FR_NVSWITCH_EXPECTED_NVLINKS_UP = 100
...
DCGM_FR_NVLINK_SYMBOL_BER_THRESHOLD = 121
DCGM_FR_IMEX_UNHEALTHY = 122  # IMEX domain or daemon status is unhealthy
DCGM_FR_FABRIC_PROBE_STATE = 123
DCGM_FR_BINARY_PERMISSIONS = 124  # Binary permissions are incorrect
DCGM_FR_GPU_RECOVERY_RESET = 125  # GPU requires reset to recover from a fault
DCGM_FR_GPU_RECOVERY_REBOOT = 126  # Node requires reboot due to GPU fault
DCGM_FR_GPU_RECOVERY_DRAIN_P2P = 127  # Peer-to-peer traffic must be drained
...
DCGM_FR_CLOCK_THROTTLE_THERMAL = DCGM_FR_CLOCKS_EVENT_THERMAL
# Deprecated: Use DCGM_FR_CLOCKS_EVENT_POWER instead
DCGM_FR_CLOCK_THROTTLE_POWER = DCGM_FR_CLOCKS_EVENT_POWER
# Deprecated: Use DCGM_CLOCKS_EVENT_VIOLATION instead
DCGM_FR_THROTTLING_VIOLATION = DCGM_FR_CLOCKS_EVENT_VIOLATION
...
DCGM_FR_FABRIC_MANAGER_TRAINING_ERROR_MSG = "Fabric Manager (Cluster UUID: %s, Clique ID: %ld, Health Mask: %#lx): %s."
...
DCGM_FR_IMEX_UNHEALTHY_MSG = "IMEX %s status is %s (%s)"
DCGM_FR_FABRIC_PROBE_STATE_MSG = "GPU %u: Fabric State is %s (%lld)."
DCGM_FR_GPU_RECOVERY_RESET_MSG = "GPU %u requires a reset to recover from a fault. Recovery action: %ld (GPU_RESET)."
DCGM_FR_GPU_RECOVERY_REBOOT_MSG = "GPU %u fault may have left the OS in an inconsistent state. Recovery action: %ld (NODE_REBOOT)."
DCGM_FR_GPU_RECOVERY_DRAIN_P2P_MSG = "GPU %u requires peer-to-peer traffic to be quiesced. Recovery action: %ld (DRAIN_P2P)."
DCGM_FR_GPU_RECOVERY_DRAIN_RESET_MSG = "GPU %u operating at reduced capacity due to a fault. Recovery action: %ld (DRAIN_AND_RESET)."
...
DCGM_FR_IMEX_UNHEALTHY_NEXT = \
    "Check IMEX installation, configuration, domain and daemon status, and network connectivity."
DCGM_FR_FABRIC_PROBE_STATE_NEXT = DCGM_FR_CUDA_FM_NOT_INITIALIZED_NEXT
DCGM_FR_GPU_RECOVERY_RESET_NEXT = "Terminate all GPU processes and reset the GPU."
DCGM_FR_GPU_RECOVERY_REBOOT_NEXT = "Reboot the operating system to restore a consistent state."
DCGM_FR_GPU_RECOVERY_DRAIN_P2P_NEXT = "Terminate GPU processes conducting peer-to-peer traffic and disable UVM persistence mode. Check GPU health status again after draining."
DCGM_FR_GPU_RECOVERY_DRAIN_RESET_NEXT = "Do not schedule new work on this GPU. Reset the GPU after existing work has drained."

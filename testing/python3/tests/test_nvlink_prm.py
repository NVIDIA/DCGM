...
def test_nvlink_prm_single_field_watch(handle, gpuIds):
    gpuId = gpuIds[0]
    link = dcgm_structs.c_dcgm_link_t()
    link.type = dcgm_fields.DCGM_FE_GPU
    link.id = gpuId
    ...
    injectedRet.values[0].value.PRMTLV_v1.dataSize = 0  # not important
    injectedRet.values[0].value.PRMTLV_v1.status = 0  # not important
    ...
    fieldId = dcgm_fields.DCGM_FI_DEV_NVLINK_PPCNT_PLR_RCV_CODES
    fieldGroupId = dcgm_agent.dcgmFieldGroupCreate(
        handle, [fieldId], "single_field_watch")
    dcgm_agent.dcgmWatchFields(
    ...
    currentValues = dcgm_agent.dcgmEntitiesGetLatestValues(
        handle, [entity], [fieldId], 0)
    assert len(
        currentValues) == 1, f"Expected 1 field value, got {len(currentValues)}"

    assert currentValues[
        0].status == dcgm_structs.DCGM_ST_OK, f"Expected status OK, got {currentValues[0].status}"
    assert not dcgmvalue.DCGM_FP64_IS_BLANK(
        currentValues[0].value.i64), f"Expected value not blank, got {currentValues[0].value.i64}"

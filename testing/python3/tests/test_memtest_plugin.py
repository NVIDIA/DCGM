...
def test_memtest_plugin_skip_if_free_mem_less_than_threshold(handle, gpuIds):
    dd = DcgmDiag.DcgmDiag(gpuIds=[gpuIds[0]], testNamesStr="memtest", paramsStr="memtest.is_allowed=True;memtest.minimum_allocation_percentage=100")
    ...
    gpuResult = next(filter(lambda cur: cur.entity.entityGroupId == dcgm_fields.DCGM_FE_GPU and cur.entity.entityId == gpuIds[0] and cur.testId == memtestId, response.results[:min(response.numResults, dcgm_structs.DCGM_DIAG_RESPONSE_RESULTS_MAX)]), None)
    ...
    gpuInfo = next(filter(lambda cur: cur.entity.entityGroupId == dcgm_fields.DCGM_FE_GPU and cur.entity.entityId == gpuIds[0] and cur.testId == memtestId, response.info[:min(response.numInfo, dcgm_structs.DCGM_DIAG_RESPONSE_INFO_MAX_V2)]), None)

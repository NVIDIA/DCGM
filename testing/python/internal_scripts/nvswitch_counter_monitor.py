...
class NvSwitchCounterMonitor:
    ...
    def _MonitorOneCycle(self):
        ...
        for entityGroupId in self._nvSwitchLatencyWatcher.values.keys():
            for entityId in self._nvSwitchLatencyWatcher.values[entityGroupId]:
                ...
                for linkIdx in range(0, self.NVSWITCH_NUM_LINKS):
                    ...
                    if self._nvSwitchLatencyWatcher.values[entityGroupId][entityId].has_key(latencyFieldId):
                        ...
                        latencyFieldId += 4;
        for entityGroupId in self._nvSwitchBandwidth0Watcher.values.keys():
            for entityId in self._nvSwitchBandwidth0Watcher.values[entityGroupId]:
                ...
                for linkIdx in range(0, self.NVSWITCH_NUM_LINKS):
                    ...
                    if self._nvSwitchBandwidth0Watcher.values[entityGroupId][entityId].has_key(bandwidth0FieldId):

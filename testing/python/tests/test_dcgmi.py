...
def test_dcgmi_diag(handle, gpuIds):
    ...
    _test_valid_args([
           ...
           ["diag", "--run", "1", "-i", allGpusCsv, "--plugin-path", "/usr/share/nvidia-validation-suite/plugins"], # verifies --plugin-path actually sees the plugins on a specified path
...
def test_dcgmi_dmon(handle, gpuIds, switchIds, instanceIds, ciIds):
    ...
    allGpusCsv = ",".join(map(str,gpuIds))
    allInstancesCsv = ",".join(map(lambda x: ("instance:" + str(x)), instanceIds))
    ...
    hugeGpuCsv = ",".join(map(str,range(0, dcgm_structs.DCGM_MAX_NUM_DEVICES*2, 1)))

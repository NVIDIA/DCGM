...
def get_nvswitch_pci_bdf():
    ...
    bdf = []
    try:
        lsPciOutput = subprocess.check_output("lspci | grep -i nvidia | grep -i bridge", shell=True)
...
def get_gpu_pci_bdf():
    ...
    bdf = []
    try:
        lsPciOutput = subprocess.check_output("lspci | grep -i nvidia | grep -i '3d controller'", shell=True)
    except subprocess.CalledProcessError as e:
        logger.error(e.message)
        return bdf

...
def is_nvvs_installed():
    '''
    Checks to see if NVVS is installed. Returns True if so. False if not.
    '''
    if os.path.isfile('/usr/share/nvidia-validation-suite/nvvs'):
        return True
    else:
        return False

def is_nvvs_supported_platform():
    '''
    Returns true if the current platform is nvvs-supported
    False if not
    '''
...
# Global to make sure we only do the work of try_load_cuda_7_5() once
tried_to_load_cuda_7_5_sos = False

def try_load_cuda_7_5():
    '''
    Try to map the cuda 7.5 libraries. Raises a TestSkipped exception on failure
    '''
    global tried_to_load_cuda_7_5_sos

    # See if we already successfully loaded the cuda 7.0 SOs
    if tried_to_load_cuda_7_5_sos:
        return

    libFilenames = ['libcudart.so.7.5', 'libcublas.so.7.5']
    for libFilename in libFilenames:
        try:
            loadedSo = ctypes.CDLL(libFilename)
        except Exception as e:
            raise TestSkipped("Unable to load Cuda library '%s'. Check LD_LIBRARY_PATH" % libFilename)

    tried_to_load_cuda_7_5_sos = True

def try_load_cuda_tooklit_libraries():
    '''
    Try to load cuda toolkit library. Raises a TestSkipped exception on failure
    '''
    try:
        loadedSo = ctypes.CDLL("libcudart.so")
    except Exception as e:
        raise TestSkipped("Unable to load Cuda Toolkit library - libcudart.so.1. Check LD_LIBRARY_PATH")

def run_with_cuda_toolkit_library_loaded():

    def decorator(fn):
        @wraps(fn)
        def wrapper(*args, **kwds):
            try_load_cuda_tooklit_libraries()

            fn(*args, **kwds)
        return wrapper
    return decorator

def run_only_on_nvvs_supported_platforms():
    '''
    Decorator to only run on platforms that support NVVS
    '''
    def decorator(fn):
        @wraps(fn)
        def wrapper(*args, **kwds):
            if not is_nvvs_supported_platform():
                skip_test("NVVS is only supported on linux x64")
...
def run_with_standalone_host_engine(timeout=15, heArgs=None, passAppAsArg=False):
    ...
    def decorator(fn):
        @wraps(fn)
        def wrapper(*args, **kwds):
            with RunStandaloneHostEngine(timeout, heArgs, profile_dir=fn.func_name) as hostengineApp:
...
def run_with_cuda_app(timeout=10000):
    def decorator(fn):
        ...
        def wrapper(*args, **kwds):
            ...
            app = apps.CudaCtxCreateAdvancedApp(params, env=test_utils.get_cuda_visible_devices_env(handle, gpuId))
            app.start(timeout * 2)
            kwds['cudaApp'] = app
            fn(*args, **kwds)
            app.terminate()
    ...
    def decorator(fn):
        ...
        def wrapper(*args, **kwds):
            ...
            if numActiveSwitches + switchCount >= dcgm_structs.DCGM_MAX_NUM_SWITCHES:
                test_utils.skip_test("unable to add fake NvSwitch with more than %d NvSwitches" % dcgm_structs.DCGM_MAX_NUM_SWITCHES)
            cfe = dcgm_structs_internal.c_dcgmCreateFakeEntities_v2()

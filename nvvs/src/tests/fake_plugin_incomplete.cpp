...
namespace
{
std::array<unsigned int, 16> g_incompleteGpuIds;
unsigned int g_incompleteNumGpus;
} //namespace
...
static dcgmDiagPluginAttr_v1 fakeIncompletePluginAttrs {};
                              ...
                              HangDetectMonitor * /* monitor */)
{
    ...
    {
        fakeIncompletePluginAttrs = *pluginAttr;
    }
    else
    {
...
void RunTest(char const * /* testName */,
             ...
             void * /* userData */)
{
    g_incompleteNumGpus = 0;
    for (unsigned int i = 0;
         i < std::min(entityInfo->numEntities, static_cast<unsigned int>(std::size(g_incompleteGpuIds)));
         i++)
    {
        if (entityInfo->entities[i].entity.entityGroupId == DCGM_FE_GPU)
        {
            g_incompleteGpuIds[i] = entityInfo->entities[i].entity.entityId;
            g_incompleteNumGpus += 1;
        }
    }
}

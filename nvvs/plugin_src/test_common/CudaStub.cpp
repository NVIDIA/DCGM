...
cudaError_t cudaDeviceDisablePeerAccess(int peerDevie)
{
    return cudaSuccess;
}
...
cudaError_t cudaMemcpyPeerAsync(void *dst, int peerId, const void *src, int srcDevId, size_t size, cudaStream_t stream)
{
    return cudaSuccess;
}

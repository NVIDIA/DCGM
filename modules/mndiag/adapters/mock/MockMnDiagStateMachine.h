...
public:
    ...
    struct Stats
    {
        ...
        bool setExpectedBinaryPathCalled          = false;
        std::string lastExpectedBinaryPath;
    };
    ...
    /**
     * Sets the expected binary path for the diagnostic process
     *
     * @param[in] path The file system path to the expected binary
     */
    void SetExpectedBinaryPath(std::string const &path) override
    {
        m_stats.setExpectedBinaryPathCalled = true;
        m_stats.lastExpectedBinaryPath      = path;

        if (m_expectedBinaryPathCallback)
        {
            m_expectedBinaryPathCallback(path);
        }
    }
    ...
    /**
     * Sets a callback to be invoked when the expected binary path is set
     *
     * @param[in] callback Function to call when SetExpectedBinaryPath is invoked
     */

    void SetExpectedBinaryPathCallback(std::function<void(std::string const &)> callback)
    {
        m_expectedBinaryPathCallback = std::move(callback);
    }

private:
    std::function<void(MnDiagStatus)> m_statusCallback;
    std::function<void(std::chrono::seconds)> m_processExecutionTimeoutCallback;
    std::function<void(std::string const &)> m_expectedBinaryPathCallback;

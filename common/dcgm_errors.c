...
dcgm_error_meta_t dcgmErrorMeta[DCGM_FR_ERROR_SENTINEL] = {
    ...
    DCGM_ERROR_TABLE_ENTRY(DCGM_FR_EMPTY_GPU_LIST, DCGM_ERROR_MONITOR),
...
const char *errorString(dcgmReturn_t result)
{
    switch (result)
    {
        ...
        case DCGM_ST_PLUGIN_EXCEPTION:
            return "Caught an exception from a diagnostic plugin";
        case DCGM_ST_NVVS_ISOLATE_ERROR:
            return "The diagnostic found an error indicating the a GPU is not fit for workload.";
        default:

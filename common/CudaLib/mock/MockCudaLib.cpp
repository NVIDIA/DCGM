...
std::string MockCudaDev::GetUuid() const
{
    return m_uuid;
}

void MockCudaDev::SetUuid(std::string const &uuid)
{
    m_uuid = uuid;
}

CUresult MockCudaLib::cuInit(unsigned int) const
{
    return CUDA_SUCCESS;
}

CUresult MockCudaLib::cuDeviceGetCount(int *count) const
{
    *count = m_mockedDevs.size();
    return CUDA_SUCCESS;
}

CUresult MockCudaLib::cuDeviceGet(CUdevice *device, int ordinal) const
{
...
CUresult MockCudaLib::cuDeviceGetUuid_v2(CUuuid *uuid, CUdevice dev) const
{
    auto const &mockedDev = CuDeviceToMockCudaDev(dev);
...
void MockCudaLib::AddMockDev(MockCudaDev const &mockdedDev)
{
    m_mockedDevs.push_back(mockdedDev);
}

MockCudaDev const &MockCudaLib::CuDeviceToMockCudaDev(CUdevice dev) const
{

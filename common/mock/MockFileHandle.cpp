...
ssize_t MockFileHandle::ReadExact(std::byte *buffer, size_t size)
{
...
void MockFileHandle::AppendToBuffer(std::span<std::byte> data)
{
    m_buffer.insert(m_buffer.end(), data.begin(), data.end());
}

ssize_t MockFileHandle::Write(void const *buffer, size_t size)
{
...
int MockFileHandle::GetErrno() const
{
    return m_errno;
}

void MockFileHandle::AddSizeToTriggerEINTR(size_t size)
{
    m_sizeToTriggerEINTR.insert(size);
}

std::vector<std::byte> const &MockFileHandle::GetWriteBuffer() const
{

...
if (CPACK_GENERATOR MATCHES "DEB")
    ...
    set(CPACK_DEBIAN_CORE_PACKAGE_DEPENDS "libc6 (>= 2.27)")

    set(CPACK_DEBIAN_CUDA_ALL_DEBUGINFO_PACKAGE FALSE)
    string(JOIN ", " CPACK_DEBIAN_CUDA_ALL_PACKAGE_DEPENDS
        "${CPACK_DEBIAN_CUDA11_PACKAGE_NAME} (= ${CPACK_DEBIAN_PACKAGE_EPOCH}:${CPACK_DEBIAN_PACKAGE_VERSION})"
        "${CPACK_DEBIAN_CUDA12_PACKAGE_NAME} (= ${CPACK_DEBIAN_PACKAGE_EPOCH}:${CPACK_DEBIAN_PACKAGE_VERSION})")

    set(CPACK_DEBIAN_CUDA11_DEBUGINFO_PACKAGE TRUE)
    set(CPACK_DEBIAN_CUDA11_PACKAGE_DEPENDS
        "${CPACK_DEBIAN_CORE_PACKAGE_NAME} (= ${CPACK_DEBIAN_PACKAGE_EPOCH}:${CPACK_DEBIAN_PACKAGE_VERSION})")
    set(CPACK_DEBIAN_CUDA11_PACKAGE_PROVIDES
        "${CPACK_PACKAGE_NAME} (= ${CPACK_DEBIAN_PACKAGE_EPOCH}:${CPACK_DEBIAN_PACKAGE_VERSION})")
    ...
    set(CPACK_DEBIAN_TESTS_DEBUGINFO_PACKAGE TRUE)
    set(CPACK_DEBIAN_TESTS_PACKAGE_DEPENDS "libc6 (>= 2.27)")

    set(SUFFIX
        "_${CPACK_DEBIAN_PACKAGE_VERSION}_${CPACK_DEBIAN_PACKAGE_ARCHITECTURE}.deb")
...
%define __brp_elfperms /bin/true")
    ...
    # - Fedora Linux >= 24
    #
    set(CPACK_RPM_CORE_PACKAGE_REQUIRES "glibc >= 2.27")
    set(CPACK_RPM_CORE_POST_INSTALL_SCRIPT_FILE "${CMAKE_CURRENT_LIST_DIR}/../scripts/rpm/postinstall")
    ...
    set(CPACK_RPM_CUDA11_PACKAGE_PROVIDES
        "${CPACK_PACKAGE_NAME} = ${CPACK_RPM_PACKAGE_EPOCH}:${CPACK_RPM_PACKAGE_VERSION}")
    # The CPack RPM generator doesn't support the Recommends spec file header.
    # As a work-around, we abuse the substitution of another header
    ...
    set(CPACK_RPM_CUDA12_PACKAGE_REQUIRES
        "${CPACK_RPM_CORE_PACKAGE_NAME} = ${CPACK_RPM_PACKAGE_EPOCH}:${CPACK_RPM_PACKAGE_VERSION}")

    set(CPACK_RPM_DEVELOPMENT_DEBUGINFO_PACKAGE TRUE)
    set(CPACK_RPM_DEVELOPMENT_PACKAGE_DEPENDS
        "${CPACK_RPM_CORE_PACKAGE_NAME} = ${CPACK_RPM_PACKAGE_EPOCH}:${CPACK_RPM_PACKAGE_VERSION}-${CPACK_RPM_PACKAGE_RELEASE}")

    set(CPACK_RPM_TESTS_DEBUGINFO_PACKAGE TRUE)
    set(CPACK_RPM_TESTS_PACKAGE_REQUIRES "glibc >= 2.27")

    foreach(component IN LISTS CPACK_COMPONENTS_ALL)
        string(TOUPPER "${component}" component)
        set(CPACK_RPM_${component}_PACKAGE_DESCRIPTION

...
def main(manualOpMode=False, embeddedHostengine=True):
    ...
    config_values = dcgmGroup.config.Get(
    ...
    try:
        ...
        for index in range(0, group_health.incidentCount):
            print("GPU ID : %d" %
                  group_health.incidents[index].entityInfo.entityId)

            print("system tested     : %d" %
                  group_health.incidents[index].system)
            print("system health     : %s" % convert_overall_health_to_string(
                group_health.incidents[index].health))
            print("system health err : %s" %
                  group_health.incidents[index].error.msg)
            print("\n")
    ...
    if nvvs_installed():
        ...
        try:
            ...
            print(
                "GPUs in the group are not compatible with each other for running diagnostics")
